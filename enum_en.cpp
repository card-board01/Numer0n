...
void update(string ansStr, int eat, int bite)
{
	int length = (int) ansStr.size();
	...
	while(!answer.empty()) {
		...
		string guessStr = answer.back();
		answer.pop_back();

		// check eat
		for(int i = 0; i < (int) length; i++) {
			if(ansStr[i] == guessStr[i]) {
				e++;
				seen[i] = 1;
			}
		}
		...
		if(e == eat && b == bite) next_answer.push_back(guessStr);
	}
